(function () {
  ...
  /* Regex-escape a literal KB phrase so it cannot break the word test. */
  ...
  function match(text) {
    ...
    if (kb) return kb;
    /* Word-boundary matching: "hi" must match the word "hi", not the
       "hi" hidden inside "which" or "this". */
    for (var i = 0; i < KB.length; i++) {
      var item = KB[i];
      if (!item.replyKey) continue;
      for (var j = 0; j < item.keys.length; j++) {
        var re = new RegExp("(^|[^a-z0-9_\u00C0-\uFFFF])" + escapeReg(item.keys[j]) + "($|[^a-z0-9_\u00C0-\uFFFF])", "i");
        if (re.test(t)) return chatT(item.replyKey);
      }
    }
  ...
  function loadCfg() {
    ...
    try {
      ...
      if (s) {
        var o = JSON.parse(s);
        /* Saved config wins for fields the user actually set; empty saved
           keys fall back to the keys shipped in chat-key.js (mirrors the
  ...
  function liveAskOnce(text) {
    ...
    /* Retry an async call a few times with backoff — Gemini is flaky,
       so a single failure must not bounce the user into the busy free
       helper. */
    function retry(fn, tries, delay) {
      var attempt = 0;
      return (function go() {
        return fn().catch(function () { return null; }).then(function (v) {
          if (v) return v;
          attempt++;
          if (attempt >= tries) return null;
          return new Promise(function (res) { setTimeout(function () { res(go()); }, delay); });
        });
      })();
    }
  ...
  function send() {
    ...
    setTimeout(function () {
      ...
      /* Unsure from the offline KB → ask the live model. */
